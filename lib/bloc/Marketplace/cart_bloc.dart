...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  FutureOr<void> updateCartProduct(
      UpdateCartProduct event, Emitter<CartState> emit) async {
        //get cart product details
      CartProductModel dbCartProduct = await CartProductRepo.getCartProduct(event.id);
    CartProductModel cartProduct = CartProductModel(
        id: event.id, amount: event.amount, priority: event.priority);
        cartProduct.productID = dbCartProduct.productID;
    int indexOfUpdatedProduct =
        state.cartProducts!.indexWhere((product) => product.id == event.id);
    if (event.amount == 0) {
    ...
    if (indexOfUpdatedProduct != -1) {
      ...
      if (selectedCartProducts.isEmpty)
        ...
        int indexOfSelectedProduct = selectedCartProducts
      ...
      totalPrice = selectedCartProducts.fold(
          0,
          (previousValue, element) =>
              previousValue +
              (element.amount! *
                  state
                      .products![state.cartProducts!.indexOf(element)].price!));
      ...
      if (message == "Successfully Updated") {
        ...
        emit(CartProductUpdated(state.selectedCartProducts!, updatedProducts,
  ...
  FutureOr<void> loadCart(LoadCart event, Emitter<CartState> emit) async {
    //see if user is guest or not
    if (AuthRepo.isGuest() == true) {
      emit(CartProductPageFromGuest());
      return;
    }
    List<CartProductModel> cartProducts = [];
    cartProducts = await CartProductRepo.getCartProducts();
    if (cartProducts.isEmpty) {
      emit(CartProductPageEmpty());
      return;
    }else{
    List<Future<MarketplaceProductModel>> productFutures =
  ...
  FutureOr<void> selectAllCartProduct(
      SelectAllCartProduct event, Emitter<CartState> emit) {
    ...
    totalPrice = selectedCartProducts.fold(
        ...
        (previousValue, element) =>
            previousValue +
            (element.amount! *
                state.products![state.cartProducts!.indexOf(element)].price!));
    emit(CartProductSelected(selectedCartProducts, state.cartProducts!,
        state.products!, totalPrice!));
  }

  FutureOr<void> selectCartProduct(
      SelectCartProduct event, Emitter<CartState> emit) {
    //find cart products based on id
    CartProductModel selectedCartProducts =
        state.cartProducts!.where((element) => element.id == event.id).first;
    totalPrice = state.totalPrice! +
        selectedCartProducts.amount! *
            state.products![state.cartProducts!.indexOf(selectedCartProducts)]
                .price!;
    emit(CartProductSelected(
        state.selectedCartProducts!..add(selectedCartProducts),
        state.cartProducts!,
        state.products!,
        totalPrice!));
  }

  FutureOr<void> unSelectAllCartProduct(
      UnSelectAllCartProduct event, Emitter<CartState> emit) {
    totalPrice = 0;
    emit(CartProductSelected(
        [], state.cartProducts!, state.products!, totalPrice!));
  }

  FutureOr<void> unSelectCartProduct(
      UnSelectCartProduct event, Emitter<CartState> emit) {
    //find cart products based on id
    CartProductModel selectedCartProducts =
        state.cartProducts!.where((element) => element.id == event.id).first;
    totalPrice = state.totalPrice! -
        selectedCartProducts.amount! *
            state.products![state.cartProducts!.indexOf(selectedCartProducts)]
                .price!;
    emit(CartProductSelected(
        state.selectedCartProducts!..remove(selectedCartProducts),
        state.cartProducts!,
        state.products!,
        totalPrice!));
  }
  ...
  FutureOr<void> cartProductPageButtonPressed(
      CartProductPageButtonPressed event, Emitter<CartState> emit) {
    bool isButtonDisabled = state.selectedCartProducts == null
        ? true
        : state.selectedCartProducts!.isEmpty;
    emit(CartProductPageButtonState(
        isButtonDisabled,
        state.selectedCartProducts!,
        state.cartProducts!,
        state.products!,
